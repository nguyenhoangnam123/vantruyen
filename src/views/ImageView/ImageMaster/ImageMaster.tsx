...
function ImageMaster() {
  ...
  const [handleGoCreate, handleGoDetail] = routerService.useMasterNavigation(IMAGE_ROUTE);
   ...
   * If export
   */
  const [handleExport] = crudService.useExport(API_IMAGE_ROUTE);
  ...
  const columns: ColumnProps<Image>[] = React.useMemo(
    () => {
...

      {
          ...
          sorter: true,
          sortOrder: getOrderTypeForTable<Image>(
            nameof(list[0].id),
            sorter,
          ),
      ...
      {
          title: translate('images.deletedAt'),
