...
import ProductImageMappingModal from 'views/ImageView/ImageDetail/ProductImageMappingModal/ProductImageMappingModal';
const {Item: FormItem} = Form;

function ProductImageMappingTable(props: ContentTableProps<Image, ProductImageMapping>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<Image, ProductImageMapping>(
    model,
    setModel,
    ...
    imageRepository.listProductImageMapping,
    imageRepository.countProductImageMapping,
    ProductImageMappingFilter,
  );
  ...
  const columns: ColumnProps<ProductImageMapping>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<Image>(nameof(dataSource[0].name), sorter),
  ...
  const tableTitle = React.useCallback(
    ...
    <CollapsibleCard title={translate(generalLanguageKeys.actions.search)} className="mb-4">
        {/* <ProductImageMappingModal title={translate('image.productImageMappingModal.title')}
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('images.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
