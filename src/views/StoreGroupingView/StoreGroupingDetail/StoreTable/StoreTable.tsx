...
function StoreTable(props: ContentTableProps<StoreGrouping, Store>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<StoreGrouping, Store>(
    model,
    setModel,
    nameof(model.stores),
  );
  ...
  const columns: ColumnProps<Store>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<StoreGrouping>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('storeGroupings.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
