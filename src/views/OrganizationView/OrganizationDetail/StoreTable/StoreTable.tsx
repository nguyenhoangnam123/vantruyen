...
function StoreTable(props: ContentTableProps<Organization, Store>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<Organization, Store>(
    model,
    setModel,
    nameof(model.stores),
  );
  ...
  const columns: ColumnProps<Store>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<Organization>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('organizations.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
