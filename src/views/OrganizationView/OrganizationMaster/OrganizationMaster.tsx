...
import './OrganizationMaster.scss';
...
function OrganizationMaster() {
  ...
  const [handleGoCreate, handleGoDetail] = routerService.useMasterNavigation(ORGANIZATION_ROUTE);
  const [pagination, sorter, handleTableChange] = tableService.useMasterTable(filter, setFilter, total, handleSearch);
  const [rowSelection, hasSelected] = tableService.useRowSelection<Organization>();
   ...
   * If export
   */
  const [handleExport] = crudService.useExport(API_ORGANIZATION_ROUTE);
  ...
  const [handleBulkDelete] = tableService.useBulkDeleteHandler(
    ...
    organizationRepository.bulkDelete,
    setLoading,
  );
  // ------------------------------------------------------------------------------------------------------------------------------------------------

  const columns: ColumnProps<Organization>[] = React.useMemo(
    () => {
      return [
      {
        ...
        render: renderMasterIndex<Organization>(pagination),
      },

      {
          title: translate('organizations.id'),
          key: nameof(list[0].id),
          dataIndex: nameof(list[0].id),
          sorter: true,
          sortOrder: getOrderTypeForTable<Organization>(
            nameof(list[0].id),
            sorter,
          ),
      ...
      {
          title: translate('organizations.code'),
      ...
      {
          title: translate('organizations.name'),
      ...
      {
          title: translate('organizations.parentId'),
      ...
      {
          title: translate('organizations.path'),
      ...
      {
          title: translate('organizations.level'),
      ...
      {
          title: translate('organizations.createdAt'),
      ...
      {
          title: translate('organizations.updatedAt'),
      ...
      {
          title: translate('organizations.deletedAt'),
      ...
      {
          title: translate('organizations.parent'),
      ...
      {
          title: translate('organizations.inverseParent'),
      ...
      {
          title: translate('organizations.stores'),
...

      {
        ...
        align: 'center',
        render(id: number, organization: Organization) {
          return (
  ...
  return (
    <div className="page master-page">
      <Card title={translate('organizations.master.title')}>
        <Card
          ...
          <Form {...formItemLayout}>
            <Row>
              ...
              <Col className="pl-1" span={8}>
                <FormItem
                  className="mb-0"
                  label={translate('organizations.id')}
                >
              ...
              <Col className="pl-1" span={8}>
                <FormItem
                  className="mb-0"
                  label={translate('organizations.path')}
                >
                    <AdvancedStringFilter
              ...
              <Col className="pl-1" span={8}>
                <FormItem
                  className="mb-0"
                  label={translate('organizations.level')}
                >
        ...
        >
          <Spin spinning={previewLoading}>
            <Descriptions title={previewModel.name} bordered>
              ...
              <Descriptions.Item label={translate('organizations.id')}>
                { previewModel?.id }
              </Descriptions.Item>


              <Descriptions.Item label={translate('organizations.code')}>
                { previewModel?.code }
              </Descriptions.Item>


              <Descriptions.Item label={translate('organizations.name')}>
                { previewModel?.name }
              </Descriptions.Item>



              <Descriptions.Item label={translate('organizations.path')}>
                { previewModel?.path }
              </Descriptions.Item>


              <Descriptions.Item label={translate('organizations.level')}>
                { previewModel?.level }
              </Descriptions.Item>
