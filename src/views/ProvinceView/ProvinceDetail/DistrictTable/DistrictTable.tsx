...
function DistrictTable(props: ContentTableProps<Province, District>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<Province, District>(
    model,
    setModel,
    nameof(model.districts),
  );

  const [
    districtFilter,
    setDistrictFilter,
  ] = React.useState<DistrictFilter>(
    new DistrictFilter(),
  );
  ...
  const columns: ColumnProps<District>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<Province>(nameof(dataSource[0].name), sorter),
        render(name: string, district: District) {
          return (
            <FormItem validateStatus={formService.getValidationStatus<District>(district.errors, nameof(district.name))}
      ...
      },
      {
        ...
        align: 'center',
        render(...params: [District, District, number]) {
          return (
            <>
