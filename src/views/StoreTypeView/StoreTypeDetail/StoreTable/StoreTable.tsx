...
function StoreTable(props: ContentTableProps<StoreType, Store>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<StoreType, Store>(
    model,
    setModel,
    nameof(model.stores),
  );
  ...
  const columns: ColumnProps<Store>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<StoreType>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('storeTypes.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
