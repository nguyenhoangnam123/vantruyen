...
function VariationMaster() {
  ...
  const [handleGoCreate, handleGoDetail] = routerService.useMasterNavigation(VARIATION_ROUTE);
   ...
   * If export
   */
  const [handleExport] = crudService.useExport(API_VARIATION_ROUTE);
  ...
  const [variationGroupingFilter, setVariationGroupingFilter] = React.useState<VariationFilter>(new VariationFilter());
  ...
  const columns: ColumnProps<Variation>[] = React.useMemo(
    () => {
      ...
      {
          title: translate('variations.variationGroupingId'),
          key: nameof(list[0].variationGroupingId),
          dataIndex: nameof(list[0].variationGroupingId),
          sorter: true,
          sortOrder: getOrderTypeForTable<Variation>(
            nameof(list[0].variationGroupingId),
            sorter,
          ),
