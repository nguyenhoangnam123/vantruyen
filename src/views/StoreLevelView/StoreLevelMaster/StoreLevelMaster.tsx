...
function StoreLevelMaster() {
  ...
  const [
    ...
    handleSearch,
    handleReset,
  ] = crudService.useMaster<StoreLevel, StoreLevelFilter>(
    StoreLevel,
    StoreLevelFilter,
    storeLevelRepository.count,
    storeLevelRepository.list,
   ...
   * If export
   */
  const [handleExport] = crudService.useExport(API_STORE_LEVEL_ROUTE);
  ...
  // ------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   // Delete handlers -------------------------------------------------------------------------------------------------------------------------------
  const [handleDelete] = tableService.useDeleteHandler<StoreLevel>(
    storeLevelRepository.delete,
    setLoading,
    list,
    setList,
  );
  const [handleBulkDelete] = tableService.useBulkDeleteHandler(
    ...
    storeLevelRepository.bulkDelete,
    setLoading,
  );
  // ------------------------------------------------------------------------------------------------------------------------------------------------

  const columns: ColumnProps<StoreLevel>[] = React.useMemo(
    () => {
      return [
      {
        ...
        render: renderMasterIndex<StoreLevel>(pagination),
      },

      {
          title: translate('storeLevels.id'),
          key: nameof(list[0].id),
          dataIndex: nameof(list[0].id),
          sorter: true,
          sortOrder: getOrderTypeForTable<StoreLevel>(
            nameof(list[0].id),
            sorter,
          ),
      ...
      {
          title: translate('storeLevels.name'),
      ...
      {
          title: translate('storeLevels.createdAt'),
      ...
      {
          title: translate('storeLevels.updatedAt'),
      ...
      {
          title: translate('storeLevels.deletedAt'),
...

      {
        ...
        align: 'center',
        render(id: number, storeLevel: StoreLevel) {
          return (
            <div className="d-flex justify-content-center">
              ...
              <button
                ...
                onClick={handleDelete(storeLevel)}
              >
  ...
  return (
    <div className="page master-page">
      <Card title={translate('storeLevels.master.title')}>
        <Card
          ...
          <Form {...formItemLayout}>
            <Row>
              ...
              <Col className="pl-1" span={8}>
                <FormItem
                  className="mb-0"
                  label={translate('storeLevels.id')}
                >
              ...
              <Col className="pl-1" span={8}>
                <FormItem
                  className="mb-0"
                  label={translate('storeLevels.name')}
                >
        ...
        >
          <Spin spinning={previewLoading}>
            <Descriptions title={previewModel.name} bordered>
              ...
              <Descriptions.Item label={translate('storeLevels.id')}>
                { previewModel?.id }
              </Descriptions.Item>


              <Descriptions.Item label={translate('storeLevels.name')}>
                { previewModel?.name }
              </Descriptions.Item>
...
export default StoreLevelMaster;
