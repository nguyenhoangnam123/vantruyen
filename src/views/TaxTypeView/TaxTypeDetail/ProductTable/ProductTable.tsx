...
function ProductTable(props: ContentTableProps<TaxType, Product>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<TaxType, Product>(
    model,
    setModel,
  ...
  const columns: ColumnProps<Product>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<TaxType>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('taxTypes.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
