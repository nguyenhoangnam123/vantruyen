...
function UnitOfMeasureGroupingMaster() {
  ...
  const [handleGoCreate, handleGoDetail] = routerService.useMasterNavigation(UNIT_OF_MEASURE_GROUPING_ROUTE);
  const [pagination, sorter, handleTableChange] = tableService.useMasterTable(filter, setFilter, total);
  const [rowSelection, hasSelected] = tableService.useRowSelection<UnitOfMeasureGrouping>();

  /**
   * If import
   */
  const [handleImport] = crudService.useImport(
    unitOfMeasureGroupingRepository.import,
    setLoading,
  );
   ...
   * If export
   */
  const [handleExport] = crudService.useExport(API_UNIT_OF_MEASURE_GROUPING_ROUTE);
  ...
  const [unitOfMeasureFilter, setUnitOfMeasureFilter] = React.useState<UnitOfMeasureGroupingFilter>(new UnitOfMeasureGroupingFilter());
  ...
  const columns: ColumnProps<UnitOfMeasureGrouping>[] = React.useMemo(
    () => {
      ...
      {
          title: translate('unitOfMeasureGroupings.unitOfMeasureId'),
      ...
      {
          title: translate('unitOfMeasureGroupings.deletedAt'),
      ...
      {
          title: translate('unitOfMeasureGroupings.unitOfMeasureGroupingContents'),
          ...
          <Form {...formItemLayout}>
            <Row>
              ...
              <Col className="pl-1" span={8}>
                <FormItem
                  className="mb-0"
                  label={translate('unitOfMeasureGroupings.id')}
                >
        ...
        >
          <Spin spinning={previewLoading}>
            <Descriptions title={previewModel.name} bordered>
              ...
              <Descriptions.Item label={translate('unitOfMeasureGroupings.id')}>
                { previewModel?.id }
              </Descriptions.Item>


              <Descriptions.Item label={translate('unitOfMeasureGroupings.name')}>
                { previewModel?.name }
              </Descriptions.Item>
