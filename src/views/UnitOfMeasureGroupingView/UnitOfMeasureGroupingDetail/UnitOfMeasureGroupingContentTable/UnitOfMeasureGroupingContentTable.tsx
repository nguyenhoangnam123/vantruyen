...
function UnitOfMeasureGroupingContentTable(props: ContentTableProps<UnitOfMeasureGrouping, UnitOfMeasureGroupingContent>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<UnitOfMeasureGrouping, UnitOfMeasureGroupingContent>(
    model,
    setModel,
  ...
  const columns: ColumnProps<UnitOfMeasureGroupingContent>[] = React.useMemo(
    () => [
      ...
      {
        title: translate('unitOfMeasureGroupingContents.name'),
        key: nameof(dataSource[0].name),
        dataIndex: nameof(dataSource[0].name),
        sorter: true,
        sortOrder: getOrderTypeForTable<UnitOfMeasureGrouping>(nameof(dataSource[0].name), sorter),
        render(name: string, unitOfMeasureGroupingContent: UnitOfMeasureGroupingContent) {
          return (
            <FormItem validateStatus={formService.getValidationStatus<UnitOfMeasureGroupingContent>(unitOfMeasureGroupingContent.errors, nameof(unitOfMeasureGroupingContent.name))}
                      help={ unitOfMeasureGroupingContent.errors?.name }
            >
              <input type="text"
      ...
      },
      {
        ...
        align: 'center',
        render(...params: [UnitOfMeasureGroupingContent, UnitOfMeasureGroupingContent, number]) {
          return (
            <>
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('unitOfMeasureGroupings.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
