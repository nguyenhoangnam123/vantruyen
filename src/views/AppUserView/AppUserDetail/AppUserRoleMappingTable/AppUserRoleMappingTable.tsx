...
function AppUserRoleMappingTable(props: ContentTableProps<AppUser, AppUserRoleMapping>) {
  ...
  const [
    ...
    appUserRepository.listAppUserRoleMapping,
    appUserRepository.countAppUserRoleMapping,
    AppUserRoleMappingFilter,
  );
  ...
  const columns: ColumnProps<AppUserRoleMapping>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<AppUser>(nameof(dataSource[0].name), sorter),
  ...
  const tableTitle = React.useCallback(
    ...
    <CollapsibleCard title={translate(generalLanguageKeys.actions.search)} className="mb-4">
        <AppUserRoleMappingModal title={translate('appUser.appUserRoleMappingModal.title')}
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('appUsers.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
