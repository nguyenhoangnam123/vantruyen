...
function ProductTable(props: ContentTableProps<ProductGrouping, Product>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<ProductGrouping, Product>(
    model,
    setModel,
  ...
  const columns: ColumnProps<Product>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<ProductGrouping>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('productGroupings.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
