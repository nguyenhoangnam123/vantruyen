...
function StoreTable(props: ContentTableProps<Ward, Store>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<Ward, Store>(
    model,
    setModel,
    nameof(model.stores),
  );
  ...
  const columns: ColumnProps<Store>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<Ward>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('wards.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
