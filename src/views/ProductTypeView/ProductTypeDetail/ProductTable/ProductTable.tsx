...
function ProductTable(props: ContentTableProps<ProductType, Product>) {
  ...
  const [
    ...
    handleAdd,
    handleDelete,
  ] = crudService.useContentTable<ProductType, Product>(
    model,
    setModel,
  ...
  const columns: ColumnProps<Product>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: getOrderTypeForTable<ProductType>(nameof(dataSource[0].name), sorter),
        ...
        <Form {...formItemLayout}>
          <Row>
            <Col className="pl-1" span={8}>
              <FormItem className="mb-0" label={translate('productTypes.id')}>
              </FormItem>
            </Col>
          </Row>
        </Form>
